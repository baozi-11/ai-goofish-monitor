...
def _row_to_task(row) -> Task:
    ...
    payload["keyword_alert_rules"] = json.loads(
        payload.pop("keyword_alert_rules_json", "[]") or "[]"
    )
    return Task(**payload)
...
class SqliteTaskRepository(TaskRepository):
        ...
        with sqlite_connection(self.db_path) as conn:
            ...
            conn.execute(
                ...
                INSERT OR REPLACE INTO tasks (
                    ...
                    decision_mode, keyword_rules_json, keyword_alert_rules_json,
                    is_running
                ) VALUES (
                    :id, :task_name, :enabled, :keyword, :description, :analyze_images,
                    :max_pages, :personal_only, :min_price, :max_price, :cron,
    ...
    def _task_values(self, task: Task) -> dict:
        ...
        values["keyword_alert_rules_json"] = json.dumps(
            [
                rule.model_dump() if hasattr(rule, "model_dump") else rule
                for rule in (task.keyword_alert_rules or [])
            ],
            ensure_ascii=False,
        )
        values.pop("keyword_rules", None)
        values.pop("keyword_alert_rules", None)
        return values
